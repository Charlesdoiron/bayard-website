...
export default async function ModerationPage() {
  ...
  return (
    <div>
      <h1 className="text-2xl font-bold tracking-tight text-gray-900">Modération</h1>
      <p className="mt-1 text-sm text-gray-600">Les annonces soumises apparaissent ici avant publication. Le vendeur est prévenu par email de la décision.</p>
              ...
              <li key={l.id} className="rounded-xl border border-gray-200 p-4 sm:p-5">
                <div className="grid gap-5 md:grid-cols-[minmax(0,2fr)_minmax(0,1fr)]">
                  <div>
                    <div className="flex gap-2 overflow-x-auto">
                      {l.images.map((src, i) => (
                        <a key={src} href={src} target="_blank" rel="noopener noreferrer" className="relative h-28 w-24 shrink-0 overflow-hidden rounded-md bg-gray-100">
                          <Image src={src} alt={`Photo ${i + 1}`} fill sizes="96px" className="object-cover" />
                        </a>
                      ))}
                    </div>
                    <h2 className="mt-3 text-lg font-semibold text-gray-900">{l.title}</h2>
                    ...
                    <p className="mt-3 whitespace-pre-line text-sm text-gray-700">{l.description}</p>
